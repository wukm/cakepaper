\documentclass[12pt]{csulb-thesis-hier}
...
\usepackage{graphicx}
\usepackage[caption=false]{subfig} % for some reason, caption=false allows left justification of captions
\usepackage{rotating}
\usepackage{bm}
%\usepackage{hyperref} remove hyperlinks per library requirement LW2019
...
\usepackage[backend=biber, style=chicago-authordate,
maxcitenames=3, mincitenames=1, maxbibnames=99, autocite=inline]{biblatex}
...
% to make interspersed comments visible, uncomment these three lines
% and comment out the three below
...
% this should be the theorem style for pretty much everything according to the library
% for all 'nonheirachical headings'
\newtheoremstyle{defths} % name
  {}% space above (NOT SURE)
  {}% space below (NOT SURE)
  {}% body font (should be normal)
  {.5in}% indent amount (half an inch)
  {\itshape}% theorem head font (should be italicized)
  {:}% punctuation after theorem head
  {.5em}% space after theorem head (NOT SURE)
  {{\itshape\thmname{#1} \thmnumber{#2} \thmnote{(#3)}}} % theorem head spec (can be left empty, meaning normal) use combination of \thmname \thmnumber and \thmnote

% newtheorem declarations that follow will have this style
% reset with \theoremstyle{plain}
\theoremstyle{defths} 
...
% change proof style to something they don't object to
\renewenvironment{proof}[1][\proofname]{{\itshape #1: }}{}
% thesis committee doesn't like qed symbols
...
\newcommand{\Am}{\ensuremath{\mathsf{A}}\xspace} % anisotropy measure
\newcommand{\Sm}{\ensuremath{\mathsf{S}}\xspace} % structureness measure
\newcommand{\Smax}{\Sm_{\max}} % max value of Structureness
...
\DeclarePairedDelimiter{\vnorm}{\Vert}{\Vert}
...
% LW2019 this fixes the figure captions. Everything is bold, period after Figure number,
% period at the end. Need to fix this for subcaptions if those are allowed
% there is apparently a ``bug'' in captions that makes them centered no matter what, and justified does nothing.
% setting caption=false in the subfig package fixes this, but then the period at the end of the figure caption
% goes away. i want to die.

\captionsetup[figure]{labelsep=period, textfont=bf, labelfont=bf, textformat=period}
\captionsetup[table]{labelsep=period, textfont=bf, labelfont=bf, textformat=period}
...
% then abstract starts at (ii).

% Removed signature page per 2019 guidelines 
% (commented out in csulb-thesis-heir.cls)
...
\setcounter{tocdepth}{0}  %%  Set how many levels to include in the TOC (it's chapters only now)
\dominitoc
\tableofcontents
...
% there should be no introductory page if there's only one appendix so comment out \HalfPage{Appendices} below
\HalfPage{Appendices}
%% Renews chapter command so type appendix files in the same way

% there should be just a half page that says APPENDIX
\myappendix{Code Listings}\label[appendix]{app:listings}
\include{manuscript/appendix}
\myappendix{Research Protocol Decision Tree}\label[appendix]{app:decisiontree}
\include{manuscript/decisiontreeappendix}


%Here is the list of cited works.
\HalfPage{Bibliography}

%\bibliographystyle{ieeetr} % this one works but it's wrong
%\bibliography{thesis_bib}
\begingroup
  \setlength\bibitemsep{12.0pt}
  \singlespacing
  \printbibliography[title={\bfseries BIBLIOGRAPHY}]
\endgroup
\end{document}
