...
\newcommand{\img}{\mathsf{I}} % discrete image (a matrix, not the math term)
...
\graphicspath{{./figures/abstract_defense/}
              ...
              {./figures/190409-width_to_scale_response_demo-unscaled-semistrict/}
              {./figures/190326-segmentation_demo_all_semistrict-ucip}
            }
\date{April 9, 2019}
...
%      \end{block}
  ...
  \begin{column}{0.48\textwidth}
    \includegraphics[height=0.4\textheight]{{{T-BN0164923-raw-mancrop}}}\\
    \includegraphics[height=0.4\textheight]{{{T-BN0164923-ctrace-mancrop}}}
        \note[item]{Mention colors are simply vessel widths (3 to 19 odds)
      are part of the tracing protocol}
    \SELF{NCS is national children's study}
    \SELF{formalin is basically formaldehyde and water, basically dried out.
          other samples i had were barium injected so the vessels were kind of
        whiter. other samples are kind of red against red. etc. etc.}
    \NOTE{Pictures taken from top down, some glare,
...
\frametitle{Appealing to Differential Geometry}
  Idealize image as a 3D surface (a graph) with $(x,y)$ spatial coordinates and intensity as the height function $h(x,y)$.
  
  \begin{equation*}
\bm{f}: \mathbb{R}^2 \to \mathbb{R}^3 \;\;\text{where}\;\;
      (x,y) \mapsto \Big(x, y, h(x,y)\Big)
  \end{equation*}
  ...
  \note[item]{This point is a lot clearer when you show multiple vessel widths, this is just a little inset}
  ...
  \NOTE{$f$ is the graph, $h$ is the height function}
  \NOTE{Technically we just map from an open set but whatever}
\end{frame}
...
\end{block}
      ...
      \note[item]{If you want to get into notation,
        you can do so as far as explicitly
...
\includegraphics[width=0.5\linewidth]{circular_trough_with_axes_flipped}}
\subfloat[Isotropic]{
...
\begin{itemize}
  \item Purpose: Filter out numerically insignificant values.
...
%\begin{block}{Frangi filter}
  ...
  \end{equation*}
%\end{block}
\includegraphics[width=0.48\textwidth]{4} % beta=0.1 gamma=1.0
\includegraphics[width=0.48\textwidth]{14} % beta=0.5 gamma=0.5 
%includegraphics[width=0.3\textwidth]{8}
\NOTE{We will soon show how choices of parameters manifest in our actual research problem here, but we have one more thing to take care of first}
\end{frame}
...
\begin{columns}
  \begin{column}{0.4\textwidth}
    ...
    \begin{itemize}
      ...
      \item Calculate in frequency space as a multiplication according to the convolution theorem.
      \item While you're at it, use a FFT.
    \end{itemize}
  \end{column}
...
\begin{center}
\includegraphics[height=0.9\textheight]{bad_gallery_rot}
\end{center}
...
\begin{frame}
\frametitle{Imperfections in Data set (1/2)}
\begin{center}
\includegraphics[width=0.8\textwidth]{annotations-on-raw}
\end{center}
\end{frame}
\begin{frame}
\frametitle{Imperfections in Data set (2/2)}
\begin{figure}
