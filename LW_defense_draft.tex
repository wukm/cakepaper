...
\frametitle{Frangi Filter Anatomy: Scale parameter}
  ...
  \note[item]{\META: You kind of need to rerun this with rescaling etc.
              to make this point easier to make)}
...
Show a number of samples, no words. Just talk about how some are better than others. Have one sample bad with bad perfusion. We'll get more into it in a minute.
\end{frame}
...
Put ucip stump (cropped here) or with an arrow. Show a masked version.
...
\TODO{When do we dump these? We never see a ring around, so it's not processed as a 0, but this region definitely isn't used to calculate Smax at the very least.}
...
\begin{center}
\includegraphics[height=0.8\textheight]{boundary_dilation_demo}
\end{center}
...
\TODO{Label rows/columns to make this readable}
\TODO{Pick something where the output is easier to tell the difference with...}
\TODO{This is fucked because of watershedding you do. Make a slide/demo for that.}
\end{frame}
...
\begin{itemize}
  ...
  \note[item]{Hessian matrix is symmetric}
