...
\section{Problem Setup in Image Processing}
	...
	A 2D grayscale image is given by a $MxN$ array of pixels, whose intensity is given by an integer value between 0 and 255.
	
	\vcleanup{I hate using L since it conflicts with the standard symbol used for the Weingarten map later. Fix these.}
	
	\begin{defn}[Image as a pixel matrix] \label{defn:image_as_pixel_matrix}
		$\mathtt{I} \in \R^{m \times n} \;\textrm{with}\; I_{ij} \in 0..255$
	\end{defn}
	
		For theoretical purposes, we wish to consider any such picture to ultimately be a sampling of a 2D continuous surface.
		We also require that this surface is sufficiently continuous as to admit the existence of second partial derivatives.
		
    \begin{defn}[Image as an interpolated surface] \label{defn:image_as_surface}
	    $h: \R^2 \to \R \; \textrm{with}\; h \in \mathcal{C}^2\left(\R^2\right).$
        \end{defn}
        
        \vcomment{the following paragraph is stupid probably. acknowledge it elsewhere or destroy it}
        It is indeed a sampling, and we could note that $h(i,j) = \mathtt{I}_{ij}$. Of course, $\mathtt{I}$ is not a perfect representation of the underlying ``content'' within the picture. There are multiple treatments of image processing that do address this \vcleanup{cite}, especially when the goal is noise reduction. However, we will be content to simply represent the pixels of \cref{defn:image_as_pixel_matrix} as the ultimate ``cause'' of the surface \cref{defn:image_as_surface}, rather than considering $\mathtt{I}$ to be a sampling of some real-world surface. \vtodo{but this contradicts the motivation behind scale space theory later. rectify}
	    
	    \hrulefill
	    
	    \section{Differential Geometry}
      	
We wish to describe the structure of an image as a surface. To do this, we develop the notion of curvature of a surface in $\R^3$ in a standard way, following \vcleanup{cite Kuhnel}
        ...
        \vcleanup{Mention when you're talking about a general surface in $\R^3$ and when it's specifically a graph and make sure it's clear which case you're dealing with and motivate why you'd want to talk about things that aren't graphs at all (to define shape operator in general). Potentially rework to not talk about non-graphs when irrelevant.
        	Although it's useful to keep some references to non-graphs to align this with "the general picture" and what
        	you'll find in a diff geo textbook, also because I already wasted time doing the hard work.}
        
        
        Given an open subset $U\subset \R^2$ and a twice differentiable function  $h: U \to \R$ (as in \cref{defn:image_as_surface})
...
%\caption{The principal eigenvectors at a ridge like structure} 
	%\vtodo{fix drawing so $u_1$ is more to scale }}
\end{center}
\end{thefigure}
