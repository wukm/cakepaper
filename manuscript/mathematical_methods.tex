...
\section{Eigenvectors of the Hessian are principal curvature directions}
	...
	\begin{itemize}
        ...
        In the context of a regular curve $c: I \to \R^3$
	        (that is, one where $c'(s) = 1 \forall s \in I$), curvature at a point $s$ is defined simply as the magnitude of the curve's acceleration: $\kappa(s) := \vnorm{c''(s)}$.
		...
		Using chain rule, we can rewrite the second coefficient as % prove chain rule?
		\begin{align}
			\inner{c''}{\nu} &=
		...
		Our final goal to is to characterize such normal curvatures and determine,
		at a given point $u \in U$, a method of determining in which directions an extremal
		normal curvature occurs. First, we need the following lemma:
        \begin{lemma}
        ...
        \begin{proof}
        	...
        	\end{defn}
        		...
        		thus we can write a matrix representation of the map $\mathcal{L}$ as the change of basis matrix %justify this
        		\begin{equation}
        		\wein := \begin{pmatrix}
