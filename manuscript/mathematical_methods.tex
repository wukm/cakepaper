...
\section{Problem Setup in Image Processing}
	...
	\vcleanup{The following stuff about the Hessian needs to go later. Introduce it in the context of differential geometry, and then consider its eigenpairs there (or even after frangi description)}
...
We wish to describe the structure of an image as a surface. To do this, we develop the notion of curvature of a surface in $\R^3$ in a standard way. 
	  ...
	  \begin{lemma}
	  	For any immersion, $\left\{\frac{\partial f}{\partial u_1} , \frac{\partial f}{\partial u_2}\right\}$ is a linearly independent set.
	  \end{lemma}
	  We have no need to prove the general case \vcomment{maybe no point in mentioning it even, also define what an immersion is?}, but
	  in our use cases the claim is clear: we can directly see that $f_{u_1}$ and $f_{u_2}$ are linearly independent in the case of a graph, as $f_{u_1} = (1,0,h_{u_2})$ and $f_{u_2} = (0,1,h_{u_2})$, which are clearly linearly independent. They are not, in general, orthogonal.
...
\section{The Frangi Filter}
    ...
    \subsection{Overview of Frangi vesselness measure}
	    ...
	    \vcomment{move to next session? or cover any necessary math here}
        The Frangi filter is a widely used \vcomment{citation needed} Hessian-based filter that relies on the principal curvatures--that is,
	...
	From this (and actually the convolution theorem) we can calculate $\tilde{F}(t)$. \vcomment{SKIP AHEAD. There's some groundwork here to copy between 213-220. We don't need the sampling theorem, since our signal is given as-is... i think}.
	...
	The transform is a continuous function with period $1 / \Delta T$. 
		...
		\vcomment{get consistent notation--either have the discrete signals be notated as  $\tilde{f}(x,y)$ or $f[x,y]$ or instead comment that it's understood}
...
\begin{itemize}
\item \vtodo{keep going, show development of 1D FFT}
\item \vtodo{show calculation counts}
\item \vtodo{how does FFTPACK implement this when sequence length is not a power of 2?}
\item \vtodo{how does scipy implement ifft? like it says in the book?}
\item \vtodo{proof of how derivatives are implemented AND USE IT IN YOUR CODE}
\item \vtodo{wraparound error, any other errors?}
\end{itemize}
...
\hrulefill
\subsection{Alternative forumulations}
