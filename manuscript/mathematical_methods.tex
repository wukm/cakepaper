...
\section{Calculating 2D Hessian}
...
In practice, the convolutions described above  are very slow for large scales ($\large \sigma$), as the size of the kernel is very large. Instead, we will perform a fast Fourier transform, which offers a speedup of $N^2$ operations vs $\mathscr{O}\left(n\cdot \log_2n\right)$ operations \vcomment{extend to 2D}.
...
The inverse DFT can actually be found via a DFT of the complex conjugate of the original signal % see p299.
