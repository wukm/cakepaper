...
In \cref{fig:cut-demo}, we showcase the effect of using our automated approach versus using the ground truth mask. The first two images demonstrate the Notably, this prevents the white background leading to false positives at the edge of the cut. To find the placental plate, we simply find the gradient (with small choice of $\sigma$) of the entire raw image (as in \cref{fig:NCSlayers-raw}), then perform a watershedding algorithm \cite{DIPGW} \cite{skimage}, with a foreground marker at the central pixel, and a background marker at a corner pixel. After watershedding, we simply label connected binary regions and identify the placental plate as the largest object. For the few images for which this technique fails, we revert to using the traced perimeter (as in \cref{fig:NCSlayers-P}).
