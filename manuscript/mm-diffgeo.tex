...
\begin{defn} The image can be given as the interpolated surface \label{def:image_as_surface}
 \begin{equation*}
 h: \R^2 \to \R
 \quad \textrm{with}\quad
...
\noindent and we can rewrite the second coefficient of \cref{eq:meusnier_acceleration} using the chain rule: % prove chain rule?
	\begin{gather}
  \begin{aligned}
		\inner{c''}{\nu} &=
      ...
      \end{aligned}
			\end{gather}
\noindent Thus, we can express the curvature at this point on our selected curve as
	\begin{align}
	...
	\end{align}
  \noindent We may compute the quantity $-\frac{\partial \nu}{\partial t}$ that appears in
...
\noindent Since $\textrm{span}\left\{ - \frac{\partial \nu}{\partial u_i}\right\}_{i=1,2}$ as a subset of $T_u f$,  we can identify a linear transformation which maps the basis
  ...
  We call this map which the Weingarten map $\wein$.
	...
	\begin{defn} The Weingarten map is the map \label{def:wein-map}
		...
		\]
	\end{defn}
\noindent That is, $\wein(\frac{\partial f}{\partial u_i}) = - \frac{\partial \nu}{\partial u_i} $ for $i=1,2$.
...
%\end{equation}
%
%We can see that as $h_x, h_y$ are close to zero, $[\delta]$ will be very close to the zero matrix, and the constant $\left(1+h_x^2 + h_y^2\right)^{-3/2}$ will be very close to 1 as well, so we should not expect the addition of a "close to 0" matrix to have much effect on the eigenvectors or eigenvalues. This intuition is confirmed by a result from Wilkinson \autocite{wilkinson-eigenvalue}, which we state without rigorous proof.
%
%% see wilkinson pg 66
%\begin{theorem}
...
%which we state without proof \autocite{matrix-analysis-horn}.
%
%\begin{theorem}
...
%\end{theorem}
%
%In the event that we do wish to rigorously compute the Weingarten map--that is, without concern for the magnitude of the gradient--we refer to \autocite{jiao2008consistent} and survey papers mentioned therein.
